...
const MobileToggle = ({
  ...
  <div className="flex items-center justify-between">
    ...
    <Button
      ...
      aria-label={`${isShieldMode ? 'Exit' : 'Enter'} privacy mode`}
    >
      {children}
    </Button>
  </div>
...
const NavLink = ({
  ...
  return (
    <Link to={item.path} aria-label={`Maps to ${item.label} page`}>
      <Button
...
export const Navigation = ({
  ...
  const navItems = [
    ...
    {
      label: isShieldMode ? 'Tools' : 'Wellness Tools',
      path: '/wellness',
      icon: Heart,
    },
    {
      label: isShieldMode ? 'Notes' : 'Journal',
      path: '/journal',
      icon: BookOpen,
    },
    {
      label: isShieldMode ? 'Audio' : 'Empathy Voice',
      path: '/voice',
      icon: Mic,
    },
          ...
          <div className="flex items-center space-x-2">
            ...
            {user ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  className="glass shadow-strong w-56"
                >
            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm" className="md:hidden">
                  <Menu className="w-5 h-5" aria-label="Open mobile menu" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-72 glass">
                <div className="flex flex-col h-full">
                  ...
                  <div className="space-y-3 pt-4 border-t">
                    <MobileToggle label="Dark Mode" onClick={onThemeToggle}>
                      {isDarkMode ? (
                        <Sun className="w-4 h-4" />
                      ) : (
                        <Moon className="w-4 h-4" />
